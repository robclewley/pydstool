...
if __name__ == '__main__':
    ...
    assert res3 == ('join', 'w')
    try:
        res4 = transition_psi(ep2, Point({'x': 1, 'z': 2, 'w': 0.1}), 0.01)
    except PyDSTool_ValueError:
        pass
    else:
        raise AssertionError

    ep1=common.args(actives=['x','y'],modulatory=['z','w'],sigma=2,relative_ratios=False)
    res1 = transition_psi(ep1, Point({'x': 1, 'y': 0.499, 'z': 0.2, 'w': 0.1}), 0.01)
    assert res1 == ('leave', 'y')
    res2 = transition_psi(ep1, Point({'x': 1, 'y': 0.601, 'z': 0.499, 'w': 0.1}), 0.01)
    assert res2 == ('join', 'z')
    ep2=common.args(actives=['x'],modulatory=['z','w'],sigma=2,relative_ratios=False)
    res3 = transition_psi(ep2, Point({'x': 1, 'z': 0.1, 'w': 0.501}), 0.01)
    assert res3 == ('join', 'w')
    try:
        res4 = transition_psi(ep2, Point({'x': 1, 'z': 2, 'w': 0.1}), 0.01)
        ...
        raise AssertionError
    ep3=common.args(actives=['x','y'],modulatory=[],sigma=2,relative_ratios=False)
    res5 = transition_psi(ep3, Point({'x': 1, 'y': 0.501}), 0.01)
    assert res5 == ('leave', 'y')

    ep1=common.args(fast=['x','y'],slow=['w','z'],order1=['v'],gamma=2)
