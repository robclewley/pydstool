...
from PyDSTool.common import _num_types, _int_types, _seq_types
...
try:
    ...
    print "dataanalysis.py:: WARNING: MDP toolbox must be installed to use PCA"
    print "You will get errors if you try to use any PCA-related functions"
    print "  imported from this toolbox"
    PCANode = None
    ...
    def to_pointset(self):
        ...
        return Pointset(indepvararray=range(self.num_bins),
                        coorddict={self.coordinate: [self.bins[ix] \
                                                        for ix in bin_ixs],
                                   'bin_limit_lo': [self.intervals[ix].get(0) \
                                                        for ix in bin_ixs],
                                   'bin_limit_hi': [self.intervals[ix].get(1) \
                                                        for ix in bin_ixs]
                                    })
