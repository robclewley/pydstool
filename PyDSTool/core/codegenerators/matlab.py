...
% Auxilliary function {name} for model {fsname}
% Generated by PyDSTool for ADMC++ target
...
class Matlab(CodeGenerator):
    ...
    def generate_auxfun(self, name, spec):
        # FIXME: do not insert "Parameter definitions" if no par definitions
        body, reusestr = self._normalize_spec(name, spec)
        code = MATLAB_AUX_TEMPLATE.format(
            name=name,
            fsname=self.fspec.name,
            args=', '.join([str(v) + '__' for v in spec[0]]),
            pardef=self.defineMany("Parameter definitions", "p", self.fspec.pars),
            reusestr=(len(reusestr) > 0) * "% reused term definitions \n" + reusestr + (len(reusestr) > 0) * "\n",
            result=body
        )
        return code, '\n'.join(code.split('\n')[:5])
    ...
    def _normalize_spec(self, auxname, auxspec):
        ...
        reusestr, auxspec_processedDict = self._processReusedMatlab(
            [auxname],
            {auxname: spec.replace(' ', '').replace('\n', '')})
        dummyQ = QuantSpec('dummy', auxspec_processedDict[auxname], treatMultiRefs=False, preserveSpace=True)
        dummyQ.mapNames(dict((v, v + '__') for v in vars_))
        body_processed = "y_ = " + dummyQ() + ";\n\n"
        return body_processed, reusestr
